...
FILE *fp;
char statefile[42], hardfile[42], *btdev, btupdown[23], addr[20], infomsg[72], cmd[32], label_on[24], label_off[24], scancmd[32], hcicmd[25]; //addressfile[34],
int state;
...
/*	if ((fp = fopen(addressfile,"r"))==NULL) exit(1);
	...
	fp = popen("/usr/bin/hcitool dev | awk '!/Devices/ {print $2}'","r");
	fgets(addr,sizeof(addr),fp);
	fclose(fp);
...
/*	addressfile[0]=0;
	...
	scancmd[0]=0;
	strcat(scancmd,"/usr/sbin/hciconfig ");
